---
title: Global effect of temperature on the concentration of secondary compounds from
  brown seaweeds
author: ''
...
In order to understand variables affecting phlototannin concentration, we used both the mean of phlonotannin concentration produced by a species as response variable. Given that brown algae occur predominantly at cooler seas and extreme temperatures should be determinant for its occurrence we chose to represent the variable temperature by maximum. Therefore, at each study locality we extracted from Bio-Oracle (Tyberghein et al. 2012, Assis et al. 2017) the following variables: sea water maximum temperature at minimum depth, salinity and photossyntetically active radiation (PAR).     
...
```{r, echo=FALSE}
hypo <- c(rep("Biogeographical hypothesis", 2), 
          "Environmental gradient hypothesis", 
          rep("Biogegraphical and environmental gradient hypothesis", 2), 
          "Null hypothesis")
models <- c("~ Temperature * Ocean", "~ Ocean", "~ Salinity + PAR", 
            "~ Temperature * Ocean + Salinity + PAR", 
            "~ Ocean + Salinity + PAR", 
            "~ 1 ") 
table1 <- data.frame(Hypotheses=hypo, Models=models)
caption1 <- "Table 1. Hypotheses and models representing each hypotheses. All models retained species and study identities as random effects as well as the Matern model for spatial autocorrelation."  
knitr::kable(table1, caption=caption1) %>%
  kable_styling(full_width = F) %>%
  #column_spec(1, bold = T) %>%
  collapse_rows(columns = 1, valign = "top")
# write.table(table1, "../results/table1.csv", sep=",", 
#             col.names=TRUE, row.names=FALSE)
```


We followed the protocol of Zuur et al. (2009) to construct and test LMMs fitted to nested and hierarchical data (i.e.replicates distributed over species and publications). We assumed the error structure of the response variables to be Gamma distributed. We then fit all models representing each hypothesis (Table 1) to each group of species separetely.
...
Florotannin concentration presented a small difference among groups (Figure 1). Desmarestiales and Fucales presented higher mean values of phlorotannin concentration. Group with small orders such as Ectocarpales, Ascoseirales, Raffsiales, Shaceraliales and Sporochnales (Other, N=11) presented lower values of phlorotannin concentration.

<center>
![Figure 1. Concentration of phlorotannin per group of brown algae.](../results/concentration_vs_order.png){width=50%}
</center>


We found that phlorotannin concentration is negatively affected by maximum temperature mediated by oceanic region only for Fucales, Laminariales and Other groups. For these groups, best model retained the interaction between temperature and oceanic region as fixed effect (Table 2). For Dictyiotales and Desmerestialies we found no effect of temperature, with Dictyiotales being affected only by oceanic region. 


```{r, echo=FALSE}
aic_tab <- read.csv("../results/all_aic.csv")
knitr::kable(aic_tab, caption = "Table 1. Model selection results for fixed effects of temperature (Temp), oceanic region (Ocean), their interaction (Temp*Ocean), saliliny (Sal) and PAR based on the random effect structure: random intercept effect of species and random intercept effect of study author. Models were ranked based on differences in Akaike’s information criterion (AIC). k is the number of parameters in the model and w is the Akaike weight of each model.", 
             col.names = c("Group", "Model", "$\\Delta$AIC", "k", "w", "AIC"), 
             digits=c(NA, NA, 2, 0, 2,2,2)) %>%
  kable_styling(full_width = F) %>%
  row_spec(seq(1, nrow(aic_tab), by=6), bold = T) %>%
  collapse_rows(columns = 1, valign = "top")
```



![Figure 2. Prediction of phlorotannin concentration at a global scale for Fucales.](../results/map01.png)

![Figure 3. Prediction of phlorotannin concentration at a global scale for Fucales.](../results/map02.png)

![Figure 4. Prediction of phlorotannin concentration at a global scale for Fucales.](../results/map03.png)

```{r, echo=FALSE}
stable1 <- read.csv("../results/S_table_01.csv")
stable1$Scientific.name <- paste0("*", stable1$Scientific.name, "*")
knitr::kable(stable1, caption="Table S1. Data incorporated in the analysis.", 
             digits=4) %>%
   kable_styling(full_width = F)
```
...
```{r, echo=FALSE}
stable2 <- read.csv("../results/S_table_02.csv")
stable2$Scientific.name <- paste0("*", stable2$Scientific.name, "*")
knitr::kable(stable2, caption="Table S2. List of species in each group.") %>%
   kable_styling(full_width = F)
```
